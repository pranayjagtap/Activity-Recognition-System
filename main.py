...
feature_combinations=["31112","31111","11121","21121","31125",  "11113","3111616","311163"]  #used for generalizing initial work
#dataframes equivalent to feature matrices for eating and cooking with 8 features
eatingcsv = pd.DataFrame( index=['eating1','eating2','eating3','eating4'], columns=['accelerometer_mean_x','accelerometer_std_x','orientation_std_y','gyro_std_y','accelerometer_max_y','orientation_rms_x','accelerometer_fft_x_17','accelerometer_fft_x_4'])
cookingcsv = pd.DataFrame( index=['cooking1','cooking2'], columns=['accelerometer_mean_x','accelerometer_std_x','orientation_std_y','gyro_std_y','accelerometer_max_y','orientation_rms_x','accelerometer_fft_x_17','accelerometer_fft_x_4'])
...
for k in  range(4):
    for j in range(len(feature_combinations)):    #iterate 7 (number of features) times and do activity as described in feature_combination list
        ...
        elif (feature_combinations[j][4] == '4'):
            operation = "min"
            df1_op_val = df1_newy.min()
            df2_op_val = df2_newy.min()
        ...
        if (k==0):
            ...
            #cookingcsv['feature' + str(k + 1)]['cooking2'] = df2_op_val
        elif (k==3):
